...
export const productsAPI = {
  ...
  getProductReviews: async (slug: string, page: number = 1, limit: number = 10): Promise<PaginatedResponse<ProductReview>> => {
    const response = await api.get<PaginatedResponse<ProductReview>>(`/products/${slug}/reviews?page=${page}&limit=${limit}`);
    return response.data;
  },

  createReview: async (slug: string, data: ReviewFormData): Promise<ProductReview> => {
    const response = await api.post<ApiResponse<ProductReview>>(`/products/${slug}/reviews`, data);
    return response.data.data!;
  },
};
...
export const categoriesAPI = {
  ...
  getCategoryProducts: async (slug: string, page: number = 1, limit: number = 12): Promise<PaginatedResponse<Product> & { data: { category: any; products: Product[] } }> => {
    const response = await api.get<PaginatedResponse<Product> & { data: { category: any; products: Product[] } }>(`/categories/${slug}/products?page=${page}&limit=${limit}`);
    return response.data;
  },
};
